...
const MOCK_PEDIDOS = [
        ...
        produtos: [
            {
                nome_produto: 'Camisa Básica',
                tecido: 'Dryfit',
                acabamentos: ['Gola Redonda', '100% Sublimada'],
                grade: [
    ...
    {
        id_pedido: 1054,
        cliente: 'Projeto Verão',
        tipo_producao: 'Sublimacao',
        cor_tecido: 'Verde',
        produtos: [
            {
                nome_produto: 'Regata Gola Redonda',
                tecido: 'Helanca Light',
                acabamentos: ['Gola Redonda'],
                grade: [
                    { tamanho: 'P', quantidade: 9 },
...
export default function CutterDashboard() {
    ...
    const [activePedidos, setActivePedidos] = useState(MOCK_PEDIDOS);
    const [historicoPedidos, setHistoricoPedidos] = useState(MOCK_HISTORICO);
    ...
    useEffect(() => {
        ...
        const loadCuttingOrders = async () => {
            ...
            try {
                ...
                setActivePedidos(MOCK_PEDIDOS);
                setHistoricoPedidos(MOCK_HISTORICO);
                setUsingMock(true);
            } finally {
            ...
            <div className="mx-auto max-w-7xl">
                <header className="mb-8">
                    <div className="mb-1 flex flex-wrap items-center justify-between gap-3">
                        ...
                        {!usingMock && saveStatus === 'saving' && (
                            <span className="rounded-full bg-blue-50 px-3 py-1 text-xs font-bold text-blue-700">
                                🔄 Salvando...
                            </span>
                        )}
                        {!usingMock && saveStatus === 'saved' && (
                            <span className="rounded-full bg-green-50 px-3 py-1 text-xs font-bold text-green-700">
                                🟢 Alterações salvas automaticamente
                            </span>
                        )}
                        {!usingMock && saveStatus === 'error' && (
